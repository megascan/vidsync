...
pub enum SyncEvent {
    ...
    Chat(crate::protocol::ChatMessage),
    Error {
        code: String,
        message: String,
    },
        ...
        loop {
            tokio::select! {
                _ = shutdown_rx.recv() => {
                    ...
                    let _ = event_tx2.send(SyncEvent::Disconnected("closed".into()));
                    break;
                }
                msg = out_rx.recv() => {
                    match msg {
                            ...
                            match serde_json::to_string(&m) {
                                Ok(s) => {
                                    if write.send(Message::Text(s.into())).await.is_err() {
                                        let _ = event_tx2.send(SyncEvent::Disconnected("send failed".into()));
                                        break;
                                    }
                                }
                ...
                incoming = read.next() => {
                    match incoming {
                        ...
                        Some(Ok(Message::Close(_))) | None => {
                            let _ = event_tx2.send(SyncEvent::Disconnected("socket closed".into()));
                            break;
                        }
                        Some(Ok(_)) => {}
                        Some(Err(e)) => {
                            let _ = event_tx2.send(SyncEvent::Disconnected(format!("{e}")));
                            break;
                        }
                    }
                }
